...
export function initFiles(store){store.db.exec(`CREATE TABLE IF NOT EXISTS shared_files(id TEXT PRIMARY KEY,parent TEXT,kind TEXT NOT NULL,name TEXT NOT NULL,note TEXT NOT NULL DEFAULT '',photo TEXT,mime TEXT,bytes BLOB,owner TEXT NOT NULL,updated TEXT NOT NULL,revision INTEGER NOT NULL DEFAULT 1); CREATE INDEX IF NOT EXISTS shared_files_parent ON shared_files(parent); CREATE TABLE IF NOT EXISTS shared_files_meta(id INTEGER PRIMARY KEY,revision INTEGER NOT NULL); INSERT OR IGNORE INTO shared_files_meta VALUES(1,0);`);}
export const filesRevision=store=>store.db.prepare('SELECT revision FROM shared_files_meta WHERE id=1').get().revision;
export const listFiles=store=>({revision:filesRevision(store),items:store.db.prepare('SELECT id,parent,kind,name,note,photo,mime,length(bytes) AS size,owner,updated,revision FROM shared_files ORDER BY kind,name COLLATE NOCASE').all()});
const get=(s,id)=>{const row=s.db.prepare('SELECT id,parent,kind,name,note,photo,mime,owner,updated,revision FROM shared_files WHERE id=?').get(id);check(row,'This item no longer exists.',404);return row;};
...
export function filesAction(store,who,d){
 ...
 if(d.action==='create'||d.action==='from-message'||d.action==='upload'){
  ...
  let kind=d.kind,name=d.name,note=d.note??'',photo=null,mime=null,bytes=null;
  ...
  const id=randomUUID(),p=parent(store,d.parent);store.db.prepare('INSERT INTO shared_files(id,parent,kind,name,note,photo,mime,bytes,owner,updated) VALUES(?,?,?,?,?,?,?,?,?,?)').run(id,p,kind,title(name),note,photo,mime,bytes,who,new Date().toISOString());touch(store);return {id};
 }
 ...
 if(d.action==='edit'){check(d.note===undefined||item.kind==='note','Only notes can be edited.');check(d.note===undefined||typeof d.note==='string'&&d.note.length<=20000,'Keep notes within 20,000 characters.');store.db.prepare('UPDATE shared_files SET name=?,note=?,revision=revision+1,updated=? WHERE id=?').run(title(d.name??item.name),d.note??item.note,new Date().toISOString(),item.id);}
