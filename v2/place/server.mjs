...
export function createApp({store,origin,secret,authFetch=fetch,ai=respond,courtAI=judge,drawAI=drawingWords,testing=false,mediaFetch=fetch,pushSend,dailyAI,memoryRenderer=renderMemoryPDF}={}){
 ...
 if(path==='/api/media/clock' &&req.method==='GET')return send(res,200,{now:Date.now()});
      ...
      if(path.startsWith('/api/messages/')&&req.method==='GET'){const m=store.db.prepare('SELECT rowid AS sequence,*,(SELECT mime FROM voices WHERE id=messages.audio) AS audioMime,(SELECT at FROM message_reads WHERE message=messages.id) AS readAt,(SELECT at FROM message_deliveries WHERE message=messages.id) AS deliveredAt FROM messages WHERE id=?').get(path.split('/').pop());check(m,'The original message is unavailable.',404);return send(res,200,m);}
