...
publishPresence(true);});return;
      ...
      if(path==='/api/voices'&&req.method==='POST'){
        ...
        check(bytes.length>16&&bytes.length<=8*1024*1024,'Record a voice message up to 8 MB.');
        const mime=bytes.subarray(0,4).equals(Buffer.from([0x1a,0x45,0xdf,0xa3]))?'audio/webm':bytes.toString('ascii',4,8)==='ftyp'?'audio/mp4':bytes.toString('ascii',0,4)==='OggS'?'audio/ogg':null;
