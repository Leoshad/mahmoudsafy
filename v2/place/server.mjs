...
export function createApp({store,origin,secret,authFetch=fetch,ai=respond,testing=false}={}){
  ...
  async function run(id){
    ...
    try{
      ...
      if(store.job(id).status==='cancelled'&&j.scope==='shared')store.db.prepare("UPDATE messages SET text=?,status='cancelled' WHERE id=?").run(output||'Echo was stopped.',id);
  ...
  async function route(req,res){Object.entries(security).forEach(([k,v])=>res.setHeader(k,v));const url=new URL(req.url,origin),path=url.pathname;
    ...
    if(path==='/api/health')return send(res,200,{ok:true,product:'Our Place',aiConfigured:!!process.env.OPENAI_API_KEY});
    ...
    check(req.method==='GET','Method not allowed.',405);const files={'/':['index.html','text/html'],'/app.js':['app.js','text/javascript'],'/style.css':['style.css','text/css'],'/suede.svg':['suede.svg','image/svg+xml'],'/scroll.js':['scroll.js','text/javascript'],'/install.js':['install.js','text/javascript'],'/manifest.webmanifest':['manifest.webmanifest','application/manifest+json'],'/icon-192.png':['icon-192.png','image/png'],'/icon-512.png':['icon-512.png','image/png']};const f=files[path];check(f,'Not found.',404);res.writeHead(200,{'Content-Type':f[1]+(f[1].startsWith('image/')?'':'; charset=utf-8')});res.end(readFileSync(join(here,'public',f[0])));
  }
