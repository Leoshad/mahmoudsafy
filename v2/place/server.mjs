...
publishPresence(true);});return;
      ...
      if(path==='/api/command'&&req.method==='POST'){
        ...
        const result=store.once(who,p.id,p,()=>{
          ...
          if(p.type==='message'){
            const value=data.text?.trim()?text(data.text,4000):'';check(value||data.image||data.audio,'Write a message or attach a photo or recording.');if(data.audio){check(!data.image,'Send the recording separately from a photo.');check(store.db.prepare('SELECT 1 FROM voices WHERE id=? AND owner=?').get(data.audio,who),'Choose a recording you uploaded.',403);}if(data.image)photoData(data.image);if(data.reply)check(store.db.prepare('SELECT 1 FROM messages WHERE id=?').get(data.reply),'The original message is unavailable.');
            store.message({id:p.id,author:who,text:value,image:data.image,audio:data.audio,reply:data.reply,aiAllowed:!s.pauses.length});return {ok:true};
          }
