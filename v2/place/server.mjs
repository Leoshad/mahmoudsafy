...
export function createApp({store,origin,secret,authFetch=fetch,ai=respond,testing=false}={}){
  ...
  async function run(id){
    ...
    try{
      ...
      store.tx(()=>{store.settle(id,usage);store.db.prepare('UPDATE jobs SET body=?,status=? WHERE id=?').run(JSON.stringify({proposals,accepted:false,latency:{firstTokenMs:first,totalMs:Date.now()-started}}),'done',id);
