...
const security={
  ...
  'Content-Security-Policy':"default-src 'self'; script-src 'self' https://www.youtube.com https://s.ytimg.com; style-src 'self'; media-src 'self' blob:; img-src 'self' blob: https://i.ytimg.com; connect-src 'self' https://www.youtube.com; frame-src https://www.youtube.com 'self';  frame-ancestors 'none'; base-uri 'none'; form-action 'self'",
  'Permissions-Policy':'camera=(), microphone=(self), geolocation=()'
};
export function createApp({store,origin,secret,authFetch=fetch,ai=respond,courtAI=judge,drawAI=drawingWords,testing=false,mediaFetch=fetch,pushSend,dailyAI,memoryRenderer=renderMemoryPDF}={}){
  ...
  initFiles(store);
  const videos=new ChatVideos(store);
  ...
  async function route(req,res){Object.entries(security).forEach(([k,v])=>res.setHeader(k,v));const url=new URL(req.url,origin),path=url.pathname;if(url.searchParams.has('code'))res.setHeader('Referrer-Policy','no-referrer');
    ...
    if(path.startsWith('/api/')){
      ...
      if(path==='/api/videos'&&req.method==='GET')return send(res,200,videos.list(who));
      if(path==='/api/videos'&&req.method==='POST'){limit('video-upload:'+who,12);return send(res,201,await videos.upload(req,who,url.searchParams.get('id')||''));}
      const videoRoute=/^\/api\/videos\/([a-f0-9-]{36})(?:\/(poster|delete|discard))?$/.exec(path);
      if(videoRoute){const [,id,action]=videoRoute;
        if(['GET','HEAD'].includes(req.method)&&(!action||action==='poster'))return videos.serve(req,res,id,who,action==='poster');
        if(req.method==='POST'&&action==='poster'){await videos.poster(req,who,id);return send(res,200,{ok:true});}
        if(req.method==='POST'&&['delete','discard'].includes(action)){const result=videos.remove(id,who,action==='discard');emit('video-removed',{id});return send(res,200,result);}
        check(false,'Method not allowed.',405);
      }
 ...
 if(path==='/api/media/clock' &&req.method==='GET')return send(res,200,{now:Date.now()});
      ...
      if(path.startsWith('/api/messages/')&&req.method==='GET'){let messageId;try{messageId=decodeURIComponent(path.split('/').pop());}catch{check(false,'Invalid message ID.',400);}const m=store.db.prepare('SELECT rowid AS sequence,*,(SELECT mime FROM voices WHERE id=messages.audio) AS audioMime,(SELECT at FROM message_reads WHERE message=messages.id) AS readAt,(SELECT at FROM message_deliveries WHERE message=messages.id) AS deliveredAt FROM messages WHERE id=?').get(messageId);check(m,'The original message is unavailable.',404);return send(res,200,m);}
...
publishPresence(true);});return;
      ...
      if(path==='/api/voices'&&req.method==='POST'){
        limit('voice:'+who,10);const p=await body(req,28*1024*1024),bytes=Buffer.from(p.data??'','base64');
        check(bytes.length>16&&bytes.length<=(p.mime==='audio/mpeg'?20:8)*1024*1024,'Choose MP3 up to 20 MB or a recording up to 8 MB.');
        const mime=isMP3(bytes)?'audio/mpeg':bytes.subarray(0,4).equals(Buffer.from([0x1a,0x45,0xdf,0xa3]))?'audio/webm':bytes.toString('ascii',4,8)==='ftyp'?'audio/mp4':bytes.toString('ascii',0,4)==='OggS'?'audio/ogg':null;
        check(mime&&String(p.mime).split(';')[0]===mime,'Unsupported recording format.');
        const total=store.db.prepare('SELECT COALESCE(SUM(length(bytes)),0) AS n FROM voices').get().n;
        check(total+bytes.length<=150*1024*1024,'Voice storage is full.',413);
        const id=randomUUID();store.db.prepare('INSERT INTO voices VALUES(?,?,?,?,?)').run(id,who,mime,bytes,new Date().toISOString());return send(res,201,{id});
      }
      if(path.startsWith('/api/voices/')&&req.method==='GET'){
        const id=path.split('/').pop(),v=store.db.prepare('SELECT * FROM voices WHERE id=?').get(id);
        check(v&&(v.owner===who||store.db.prepare('SELECT 1 FROM messages WHERE audio=?').get(id)),'Recording not found.',404);
        const bytes=Buffer.from(v.bytes),range=req.headers.range;let start=0,end=bytes.length-1;
        res.setHeader('Accept-Ranges','bytes');
        if(range){const m=/^bytes=(\d+)-(\d*)$/.exec(range);if(!m||Number(m[1])>=bytes.length||m[2]&&Number(m[2])<Number(m[1])){res.writeHead(416,{'Content-Range':'bytes */'+bytes.length});return res.end();}start=Number(m[1]);if(m[2])end=Math.min(end,Number(m[2]));res.setHeader('Content-Range',`bytes ${start}-${end}/${bytes.length}`);}
        res.writeHead(range?206:200,{'Content-Type':v.mime,'Content-Length':end-start+1});return res.end(bytes.subarray(start,end+1));
      }
      ...
      if(path==='/api/command'&&req.method==='POST'){
        ...
        const result=store.once(who,p.id,p,()=>{
          ...
          if(p.type?.startsWith('media.')){if(verifiedTrack)data.track=verifiedTrack;mediaChange(s,who,p.type,data);recordListening(store,s);store.save(s);return {ok:true};}
          if(p.type==='message'){
            const value=data.text?.trim()?text(data.text,4000):'';check(value||data.image||data.audio||data.video,'Write a message or attach a photo, video or recording.');if(data.video){check(!data.image&&!data.audio,'Send the video separately from other attachments.');const v=videos.access(data.video,who);check(v.owner===who,'Choose a video you uploaded.',403);}if(data.audio){check(!data.image,'Send the recording separately from a photo.');check(store.db.prepare('SELECT 1 FROM voices WHERE id=? AND owner=?').get(data.audio,who),'Choose a recording you uploaded.',403);}if(data.image)photoData(data.image);if(data.reply)check(store.db.prepare('SELECT 1 FROM messages WHERE id=?').get(data.reply),'The original message is unavailable.');
            store.message({id:p.id,author:who,text:value,image:data.image,audio:data.audio,video:data.video,reply:data.reply,aiAllowed:!s.pauses.length});return {ok:true};
          }
          ...
          if(p.type==='message.pin'&&data.value){const m=store.db.prepare('SELECT * FROM messages WHERE id=?').get(data.id);check(m&&m.status==='sent','Only a sent shared message can be pinned.',404);data.text=m.text||(m.audio?'Voice message':'Shared photo');data.author=m.author;}
