...
export function attentionEvents(before,after,actor){
 ...
 for(const post of after.items??[]){const old=before.items?.find(x=>x.id===post.id),target={tab:'space',post:post.id};if(!old){if(post.daily?.notify)people.forEach(n=>add('daily:'+post.id+':'+n,n,'Echo shared a new post on your wall.',target,'daily'));if(people.includes(post.by))add('post:'+post.id,other(post.by),post.by+' shared a post in Our Space.',target,'wall');continue;}
