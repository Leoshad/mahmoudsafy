...
Echo model routing lives in ai-models.mjs. Existing budget caps remain unchanged. echo_usage records model, task, input/output tokens and conservative estimated cost (uncached rates plus search fees; 25% guard for budget accounting). This is not the provider invoice. Historical token usage cannot be reconstructed.

## Memory book PDF export
Our Chat Menu > Export memories as PDF downloads an authenticated A5 PDF, for chat or all saved visible content, with local-time day/range filters. Text remains selectable, Arabic uses bundled fonts, images are embedded without cropping, replies link to their original message when included, and ordinary links remain clickable. Full export includes posts/comments, gallery/notes, saved competitive results, visible activity answers, dates, court records, and the current shared drawing for unfiltered exports. Attachments and recordings use authenticated original links, not embedded playback. Unknown-date historical activities are included only in unfiltered exports. Deleted or never-recorded history cannot be reconstructed.
Shared playback events persist in SQLite from this release onward when both participants have joined and the shared session is playing; this is a playback-state record, not proof of audible playback on both devices. Invitations alone are not recorded.
PDF generation blocks external browser requests and script execution, allows one render at a time, closes on cancellation, and rejects oversized selections explicitly (10,000 messages / 32 MiB source images) instead of truncating. The synthetic PDF renderer runs as a build gate. No extra hosted service or AI call is used.

### Shared touch
At the bottom of Our Chat, a direct upward swipe starting in the strip from the top of the last message bubble to the bottom viewport edge, across the full viewport width and reaching its midpoint reveals one fingerprint over the unchanged chat. Reversing or releasing before the threshold stays local. Completion opens the shared overlay directly through authenticated SSE, with no invitation or notification. The partner sees it while the chat is visible and focused. A stationary 350 ms hold still owns the Balloon gesture; the direct swipe claims movement before 300 ms and cancels the balloon candidate and pending message long press. Small sideways finger drift is tolerated; the fingerprint layers are mirrored horizontally. Neither overlay starts while the other owns the gesture.

Both authenticated accounts must sustain a touch for roughly four seconds. The server owns progress, uses 400 ms client pulses and 1.2 s leases, and rejects reordered requests and duplicate devices for the same live hand. Release, blur, background and stream disconnect clear touch; abrupt network loss is detected by the lease (not instant). Completion latches both illuminated names and “Feel my touch. I’m here with you ♥” until X closes the shared moment. Incomplete idle moments expire after 90 seconds. These are screen touches, not biometric thumb authentication. The active gesture is ephemeral. Each completed moment saves one shared “A shared touch ♥” chat event with its completion time, included in chat and full PDF exports. Incomplete attempts create no record; completed moments do not send push notifications. There is no swipe hint.


Latest Shared Touch behavior: anyone may fully open a local scene regardless of partner readiness. Only clients in focused chat at the instant of opening join the shared scene; the audience is fixed and late arrivals never see it. Solo scenes are not broadcast or saved. Partner departure or shared-session expiry converts the remaining scene to local mode, without dismissing it; each person closes their own scene with X. Both live touches are still required for completion and a saved memory.
...
MP3 attachments: tap + to choose Photo or Audio file (MP3 up to 20 MB); holding + still records voice. Audio files reuse authenticated audio storage (150 MB shared cap) and byte-range playback. Each listener explicitly starts their own independent persistent player, outside chat rendering, with Media Session controls. Tab switching does not destroy it; logout and X stop it. Background/locked-screen playback is best effort and controlled by the mobile OS. Presence remains independent of audio playback.


## 2026-09-23 scheduled delivery, audio cards and touch search

- Daily preparation starts up to one hour early. Research accepts explicit HTTPS Markdown/article links as well as API URL annotations; every research article is still fetched through the SSRF-safe verifier before editorial review. News keeps its 48-hour publication requirement. Search attempts and existing budget caps remain bounded.
- At a due slot, failed/interrupted/retrying/running requests can publish an offline editorial reserve independently of the AI job lock. Budget/provider outages can also use the reserve without a paid request. An afternoon substitute explicitly says it is not current news. Delivered slots are idempotent across restart; late writer success/failure cannot duplicate or replace the reserve. Just Us, disabled slots, future-only settings, and the existing 500-item wall limit remain respected. The 15-second scheduler depends on the service running; this is not an absolute uptime or real-phone push guarantee.
...
Video playback refinement: chat previews hide filenames and external download links, show the send time inside the bubble, and open an inline native player across the viewport over a dimmed chat. Fullscreen is user initiated; tapping outside closes and releases playback without scrolling the chat. Native player download controls remain browser provided. Long press on video previews and music cards opens the shared reaction/reply/pin picker; seek sliders and native media controls keep their normal gestures. Mobile Chromium coverage verifies no pre-play transfer, hold without accidental playback, viewport player, outside dismissal, and remote deletion.

Search/history correction: navigating to an old search result, pin or reply now loads every history page between that message and the loaded conversation. Search results are no longer temporary feed rows. Closing search removes highlights and keeps the current reading position and loaded messages. The persisted sequence and original createdAt values are unchanged. Run `node tests/search-history-browser-check.mjs` for two-account mobile verification across 210 dated messages, search close, reactions, pins, earlier paging, live updates and reload.

Domino table and fullscreen chat update: connected serpentine rows adapt automatically to the available board, with crosswise doubles, uniform spacing and a 20px minimum short tile edge (32px preferred). The board reserves readable height on short screens and uses a landscape arrangement; no pan/zoom controls. Rules, saved tile sequence, scores and game state are unchanged. Fullscreen Domino, Ocho, drawing and race headers display the shared unread count; opening chat uses the game’s existing pause/exit behavior, and counts clear through the existing read acknowledgements. `node tests/domino-auto-browser-check.mjs` verifies all 28 tiles fit in solo/shared at 390×740, 360×640 and 740×390, and real message arrival/read handling in Domino and Ocho.
