...
Shared Touch now requires fresh, authenticated chat readiness from both accounts (one-second updates, 3.5-second loss timeout). Leaving chat, blurring, hiding or disconnecting cancels the moment; rejected and canceled moments cannot replay. The fingerprint artwork is 10% larger. Sudden network loss is detected by the lease rather than instantaneously.
