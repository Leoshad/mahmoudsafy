...
Domino layout correction: restores the original stable, connected path and removes the replacement rows and connector lines. Existing tile poses stay fixed when either endpoint grows. Tile short edges stop shrinking at 20px (original preferred size: 25px); the table reserves the necessary height to avoid clipping. On screens too short for that path, the game page can scroll; no board pan/zoom controls. Fullscreen unread-chat badges are retained. Rules, stored chain order, chat history and timestamps are unchanged.
