...
export class Store {
  constructor(path){
    ...
    const recovered=this.state();let changed=recovered.competition?.schema!==1;for(const item of recovered.items)for(const c of item.comments??[])if(c.by==='Echo'&&c.status==='streaming'){c.status='interrupted';c.text=c.text||'Echo was interrupted. You can ask again.';changed=true;}for(const c of recovered.court?.cases??[])if(c.pending){c.pending=null;c.error='Echo was interrupted. Your case is saved. Retry when ready.';c.revision++;recovered.version++;changed=true;}if(changed)this.save(recovered);
  }
