...
function play(s,g,who,id,color,now){
 ...
 g.hands[who].splice(g.hands[who].indexOf(c),1);g.pile.push(c);for(const viewer of g.players)if(g.peeks[viewer]===c.id)delete g.peeks[viewer];g.color=wild(c)?color:c.color;g.drawn=null;g.passes=0;g.move++;g.lastCard={...c,by:who,move:g.move};g.last=who+' played '+ochoLabel(c)+'.';g.turn=other(g,who);
