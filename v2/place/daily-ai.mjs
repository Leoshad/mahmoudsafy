...
export async function dailyGenerate({kind,variant,history=[],post,signal,previousFailure='',attempt=1,now=Date.now(),fetcher=fetch}){
 ...
 try{
  ...
  let review;try{review=await reviewDaily({value,kind,variant,history,signal,now,fetcher});}catch(e){if(e.usage){usage.input_tokens=(usage.input_tokens||0)+(e.usage.input_tokens||0);usage.output_tokens=(usage.output_tokens||0)+(e.usage.output_tokens||0);}else{e.usage=undefined;e.unknownReviewCost=true;}throw e;}
