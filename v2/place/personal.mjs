...
export function personalSnapshot(s,who,now=Date.now()){
 ...
 const profiles=Object.fromEntries(names.map(n=>{const v=p.profiles[n]??{},birthday=v.birthday;const today=localDay(now,v.zone||'Africa/Cairo');const age=birthday?Number(today.slice(0,4))-Number(birthday.slice(0,4))-(today.slice(5)<birthday.slice(5)?1:0):null;return [n,{name:n,bio:v.bio??'',photo:v.photo??null,birthdayMonthDay:birthday?.slice(5)??null,age:birthday&&(who===n||v.showAge)?age:null,...(who===n?{birthday:birthday??'',showAge:v.showAge??true,zone:v.zone??'Africa/Cairo',revision:v.revision??0}:{})}];}));
 ...
 return {profiles,dates,reminders:reminders(p,who,now),zones:ZONES};
}
export function personalChange(s,who,type,d,now=Date.now()){
 ...
 if(type==='personal.profile'){
  ...
  p.profiles[who]={bio:clean(d.bio??'',180),birthday,showAge:d.showAge,zone:d.zone,photo:d.photo||null,revision:old.revision+1,birthdaySince:old.birthday===birthday&&old.zone===d.zone?old.birthdaySince:now,birthdayRevision:(old.birthdayRevision??0)+(old.birthday===birthday&&old.zone===d.zone?0:1)};
