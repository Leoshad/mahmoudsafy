...
export function updateCrown(s){
 if(s.competition?.schema===2)return;
 const previous=s.competition;
 const c=s.competition={schema:2,totals:empty(),games:{},results:{},holder:null,revision:0,events:[]};const history=[];
 for(const [key,title] of [['domino','Dominoes'],['ocho','Ocho']]){const old=previous?.games?.[key]?{wins:previous.games[key].wins,history:Object.values(previous.results||{}).filter(r=>r.game===key).map(r=>({...r,id:r.matchId,status:'complete'}))}:s[key]?.records?.shared;const wins={...empty(),...old?.wins};c.games[key]={title,wins};for(const n of players)c.totals[n]+=wins[n];
 ...
 if(previous&&Object.entries(previous.games||{}).every(([key,g])=>['domino','ocho'].includes(key)||players.every(n=>!g.wins?.[n]))){c.holder=leader(c.totals)??previous.holder??c.holder;c.revision=previous.revision||0;c.events=previous.events||[];}
}
...
export function recordCompetitiveResult(s,{game,title,matchId,participants,mode,status,winner},now=Date.now()){
 updateCrown(s);
 if(!['domino','ocho'].includes(game)||mode!=='shared'||status!=='complete'||!players.includes(winner)||participants?.length!==2||!players.every(n=>participants.includes(n)))return false;
...
export function crownSnapshot(s,who){if(!players.includes(who))return null;updateCrown(s);const c=s.competition;return {mode:'shared',holder:c.holder,totals:{...c.totals},revision:c.revision,events:c.events,breakdown:Object.fromEntries(Object.entries(c.games).map(([k,g])=>[k,g.wins])),gameTitles:Object.fromEntries(Object.entries(c.games).map(([k,g])=>[k,g.title]))};}
