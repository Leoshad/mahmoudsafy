import {normalizeActivityTarget} from './activity-participants.mjs';
...
import {STRONG_MODEL,modelFor} from './ai-models.mjs';
export const MODEL=STRONG_MODEL;
...
export async function respond({actor,prompt,context,image,purpose='chat',signal,onText,fetcher=fetch,youtube,resolveContent=recommend,verifyLinks=verifyReplyLinks}){
  ...
  if(canRecommend&&recommendation){
    ...
    const result=await resolveContent(args.requests,{youtube,fetcher,signal,model,language:echoLanguage(prompt)});
  ...
  for(const out of completed.output??[]){if(out.type!=='function_call')continue;const args=correctSafy(JSON.parse(out.arguments));
    if(out.name==='start_quiz'){args.target=normalizeActivityTarget(prompt,actor,args);check(['Mahmoud','Safy','Both'].includes(args.target),'Invalid live activity.');proposals.push({type:'start',target:args.target,title:String(args.title).slice(0,200),questions:questions(args.questions)});}
