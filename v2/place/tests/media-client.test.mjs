...
function client(who,room,clients){
 ...
 class Element{
  constructor(tag='div'){this.tag=tag;this.children=[];this.hidden=false;this.value='';this.parentNode=null;this.dataset={};const classes=new Set();this.classList={add:n=>classes.add(n),remove:n=>classes.delete(n),toggle:(n,v)=>{if(v??!classes.has(n))classes.add(n);else classes.delete(n);},contains:n=>classes.has(n)};}
 ...
 const context={IntersectionObserver:class{constructor(fn){intersection=fn;}observe(){}disconnect(){}},document,console,URL,location:{origin:'https://test.example'},performance:{now:()=>now},Date,setTimeout,clearTimeout,setInterval:fn=>{interval=fn;},YT:{Player}};context.window=context;vm.createContext(context);vm.runInContext(script,context);
 const c={who,context,get,document,made:()=>made,player:()=>latestPlayer,commands:()=>commands,scroll:away=>intersection([{isIntersecting:!away}]),advance:()=>{now+=2000;},tick:()=>interval(),sync(){context.OurMedia.sync({who,media:structuredClone(room.media??null),serverNow:Date.now()});}};clients.push(c);
...
test('client consent: preview only locally, invite without remote player, join, shared pause and hidden-tab stop',async()=>{
 ...
 a.scroll(true);assert.equal(a.player().getPlayerState(),1);assert.equal(a.get('#media-player-box').classList.contains('media-floating'),true);a.scroll(false);assert.equal(a.get('#media-player-box').classList.contains('media-floating'),false);
