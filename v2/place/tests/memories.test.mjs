...
test('PDF crown follows either current holder and full export includes crown history',()=>{
 const db=new Store(':memory:');initMemories(db);try{for(const holder of ['Safy','Mahmoud',null]){const s=db.state();s.competition={schema:2,holder,totals:{Mahmoud:3,Safy:3},games:{},results:{},events:[],revision:1};db.save(s);db.db.prepare('DELETE FROM messages').run();for(const author of ['Mahmoud','Safy','Echo'])db.message({id:author,author,text:'hello',status:'sent'});const data=memoryData(db,'Mahmoud',options('scope=all'));assert.equal(data.crown.holder,holder);const html=memoryHTML(data);assert.equal((html.match(/class="portrait champion"/g)||[]).length,holder?1:0);if(holder)assert.match(html,new RegExp('id="m-'+holder+'"><span class="portrait champion"'));assert.match(html,/id="crown"/);assert.match(html,/lifetime totals at export time/);assert.ok(!memoryHTML({...data,scope:'chat'}).includes('id="crown"'));}}finally{db.close();}
});
