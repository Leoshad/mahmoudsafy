...
test('voice uploads are private until sent, durable in messages, seekable and included in export',async()=>{
 ...
 try{
  ...
  const bytes=Buffer.concat([Buffer.from([0x1a,0x45,0xdf,0xa3]),Buffer.alloc(60,7)]);
  const upload=await req('mahmoud','/api/voices',{mime:'audio/webm;codecs=opus',data:bytes.toString('base64')});assert.equal(upload.status,201);const {id}=await upload.json();
  ...
  const state=await (await req('safy','/api/state')).json();assert.equal(state.messages.length,1);assert.equal(state.messages[0].audio,id);
  const sound=await req('safy','/api/voices/'+id,null,{Range:'bytes=4-9'});assert.equal(sound.status,206);assert.equal((await sound.arrayBuffer()).byteLength,6);assert.equal(sound.headers.get('content-type'),'audio/webm');
