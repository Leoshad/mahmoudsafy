...
function client(s,who='Mahmoud'){
 ...
 const root=node(),commands=[];const context={document:{activeElement:null},el,btn:(t,p,fn)=>{const n=el('button',t,p);n.onclick=fn;return n;},state:{...project(s,who),who},command:async(type,data)=>{commands.push(type);change(s,who,type,data);},sync:async()=>{context.state={...project(s,who),who};context.renderActivity(context.state.activity,root);}};
...
for(const who of ['Mahmoud','Safy'])test('shared question gives each account its own controls and waits after submission: '+who,async()=>{
 const s=initial();change(s,'Mahmoud','quiz.launch',{host:'Echo',target:'Both',questions:[{q:'Choose?',options:['Socks','Spoons'],correct:-1}]});const c=client(s,who);
 assert.match(c.text(),/A question for Mahmoud & Safy/);await c.click('Socks');assert.match(c.text(),/Your answer is saved/);assert.equal(s.activity.index,0);
 assert.equal(walk(c.root).some(n=>n.tag==='button'&&n.textContent==='Spoons'),false);
});
test('single-recipient question does not show a misleading answer field to the other account',()=>{
 const s=initial();change(s,'Mahmoud','quiz.launch',{target:'Safy',questions:[{q:'Choose?',options:[],correct:-1}]});const c=client(s,'Mahmoud');assert.match(c.text(),/Waiting for Safy/);assert.equal(walk(c.root).some(n=>n.tag==='input'),false);
});
