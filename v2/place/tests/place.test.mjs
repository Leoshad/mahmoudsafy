...
test('provider streams text immediately and validates quiz tool output',async()=>{
 ...
 const r=await respond({actor:'Safy',prompt:'Quiz',context:'',onText:t=>observed+=t,fetcher:async(_url,opts)=>{const p=JSON.parse(opts.body);assert.equal(p.stream,true);assert.equal(p.store,false);assert.equal(p.model,'gpt-6-sol');return new Response(eventList.map(e=>'data: '+JSON.stringify(e)+'\n\n').join(''));}});assert.equal(observed,'Hello');assert.equal(r.proposals[0].questions.length,2);
});
test('Our Space creation requests a usable item proposal without changing the model or adding a second AI call',async()=>{
 ...
 const result=await respond({actor:'Mahmoud',prompt:'Plan our evening',purpose:'space',context:'',onText(){},fetcher:async(_url,opts)=>{
  calls++;const body=JSON.parse(opts.body);assert.equal(body.tool_choice.name,'propose_item');assert.equal(body.model,'gpt-6-sol');assert.equal(body.parallel_tool_calls,false);assert.match(body.instructions,/ready-to-use/);
