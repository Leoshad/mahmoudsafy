...
function render(text,author='Echo'){
 ...
 const root=node('p');ctx.renderMessageText(root,{author,text,status:'streaming'});
...
for(const author of ['Mahmoud','Safy'])test(author+' links are active and surrounding text stays literal',()=>{
 const text='شوف **ده** https://youtu.be/abcdefghijk?t=20، و https://example.com/article.';
 const {root,links}=render(text,author);assert.equal(root.textContent,text);assert.equal(links.length,2);assert.equal(links[0].href,'https://youtu.be/abcdefghijk?t=20');assert.equal(links[1].href,'https://example.com/article');for(const a of links){assert.equal(a.target,'_blank');assert.equal(a.rel,'noopener noreferrer');}
 assert.equal(render('https://user:pass@example.com [bad](javascript:alert)',author).links.length,0);
});
