...
function deal(g){
 const deck=shuffled();g.hands={[g.players[0]]:deck.splice(0,7),[g.players[1]]:deck.splice(0,7)};g.stock=deck;g.chain=[];g.passes=0;g.result=null;g.turn=g.players[(g.round-1)%2];g.status='active';g.last={text:g.turn+' starts the round.'};
}
...
function play(g,who,tile,side){
 ...
 const laid={id:t.id,a,b,by:who};if(side==='left')g.chain.unshift(laid);else g.chain.push(laid);
...
function bots(g){
 for(let n=0;g.status==='active'&&g.turn==='Computer'&&n<30;n++){
  ...
  if(move){play(g,'Computer',move.tile,move.side);break;}
  if(g.stock.length)g.hands.Computer.push(g.stock.pop());else{pass(g,'Computer');break;}
 }
}
...
export function dominoChange(s,who,type,p={},now=Date.now()){
 ...
 if(type==='domino.create'){
  ...
  check(!old||['finished','ended','declined'].includes(old.status)||old.status==='waiting'&&old.expires<=now,'Finish or leave your current game first.',409);
  const players=[who,p.mode==='solo'?'Computer':names.find(n=>n!==who)];
  const g={id:randomUUID(),revision:1,owner:who,mode:p.mode,difficulty:p.mode==='solo'?p.difficulty:null,players,status:'waiting',expires:now+600000,round:1,scores:Object.fromEntries(players.map(n=>[n,0])),hands:{},chain:[],stock:[],turn:null,result:null,last:{text:'Invitation sent.'}};
  ...
  if(type==='domino.accept'||type==='domino.decline'){
   ...
   check(g.status==='active'||g.status==='waiting'||g.status==='finished','This game has ended.',409);
   check(g.status!=='waiting'||g.owner===who,'Accept or decline this invitation.',403);
   g.status='ended';g.turn=null;g.last={text:who+' left the game.'};
