...
let host,opened=false,version=0,timer,mark,query='',index=0,total=0,selected=null,activeMessage=null,pages=new Map(),noticeTimer,clearTimer,lastNotice='',lastAt=0;
...
async function show(next,token=++version){
 ...
 clearHighlights();selected=message.id;activeMessage=message;host.insert(message);highlight();
...
function close(){if(!opened)return;opened=false;++version;clearTimeout(timer);$('chat-search').hidden=true;$('chat-search-open').setAttribute('aria-expanded','false');clearHighlights();selected=null;activeMessage=null;pages.clear();host.clear?.();if(mark){PlaceScroll.restore($('timeline'),mark);mark=null;}}
...
window.OurChatTools={init,notice,highlight,close,reset,messages:()=>activeMessage?[activeMessage]:[]};
})();
