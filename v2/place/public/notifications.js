(()=>{'use strict';
let who=null,registration=null,bound=null,pending=null,busy=false,epoch=0;
const client=crypto.randomUUID(),supported='serviceWorker'in navigator&&'PushManager'in window&&'Notification'in window;
const $=s=>document.querySelector(s),dialog=$('#notifications-dialog'),status=$('#notifications-status'),enable=$('#notifications-enable'),disable=$('#notifications-disable');
...
function presence(){if(who)request('/presence',{client,visible:document.visibilityState==='visible'},true).catch(()=>{});if(document.visibilityState==='visible')clearShown();}
...
document.addEventListener('visibilitychange',presence);window.addEventListener('pageshow',presence);window.addEventListener('pagehide',()=>{if(who)request('/presence',{client,visible:false},true).catch(()=>{});});
setInterval(()=>{if(who&&!document.hidden)presence();},15000);
window.OurNotifications={sync(state){if(who!==state.who){who=state.who;epoch++;bound=null;presence();bind().catch(()=>{});}deliver();},reset(){if(who)request('/presence',{client,visible:false},true).catch(()=>{});who=null;bound=null;epoch++;dialog.close();clearShown().catch(()=>{});}};
