...
window.OurWall=function({getState,api,command,sync,goto,info,error,el,btn,categories,getFilter,setFilter,openSource}){
 ...
 let draftOwner=null,postDrafts={},draftActive=false;
 function loadDrafts(){const who=getState()?.who;if(who===draftOwner)return;draftOwner=who;postDrafts={};draftActive=false;try{const saved=JSON.parse(localStorage.getItem('our-place-wall-drafts:'+who)||'{}');if(saved&&typeof saved==='object'&&!Array.isArray(saved))postDrafts=saved;}catch{}}
 function persistDrafts(){try{localStorage.setItem('our-place-wall-drafts:'+draftOwner,JSON.stringify(postDrafts));return true;}catch{return false;}}
 function saveDraft(){if(!draftActive||!draftOwner)return;const d={editing:structuredClone(editing),photos:[...photos],type:$('#item-type').value,title:$('#item-text').value,steps:$('#wall-steps').value,link:$('#wall-plan-link').value,time:$('#wall-plan-time').value,ai:$('#item-ai').checked,receipt:postRequest,at:Date.now()};
  if(!d.title.trim()&&!d.photos.length&&!d.steps.trim()&&!d.link.trim()&&!d.time&&!editing.id)return;
  postDrafts[editing.id||'new']=d;const stored=persistDrafts();$('#wall-draft-status').textContent=stored?'Draft saved on this device.':'Draft kept while this app stays open.';draftBanner();
 }
 function draftBanner(){loadDrafts();$('#wall-resume-draft').hidden=!Object.keys(postDrafts).length;}
 function clearDraft(){delete postDrafts[editing.id||'new'];persistDrafts();draftActive=false;draftBanner();}
 $('#item-form').oninput=saveDraft;$('#item-form').onchange=saveDraft;
 $('#wall-resume-draft').onclick=()=>{loadDrafts();const d=Object.values(postDrafts).sort((a,b)=>b.at-a.at)[0];if(d)edit(d.editing);};
 const discardDialog=el('dialog',null,$('#our-place-trial'),'wall-discard-dialog');el('h3','Discard this draft?',discardDialog);el('p','Your unpublished changes will be removed. Published posts stay unchanged.',discardDialog);const discardButtons=el('div',null,discardDialog,'row');btn('Keep writing',discardButtons,()=>discardDialog.close());btn('Discard draft',discardButtons,()=>{clearDraft();discardDialog.close();epoch++;editing={};photos=[];$('#item-form').reset();goto('space',true);});
 $('#wall-discard-draft').onclick=()=>{if(uploading){info('Please wait for the photo upload to finish.');return;}discardDialog.showModal();};
 ...
 const planState=i=>i.plan??{responses:people.includes(i.by)?{[i.by]:{value:'yes'}}:{},schedule:null};
 const localZone=()=>Intl.DateTimeFormat().resolvedOptions().timeZone;
 function planDate(v){return new Intl.DateTimeFormat('en',{dateStyle:'full',timeStyle:'short'}).format(new Date(v));}
 const planDialog=el('dialog',null,$('#our-place-trial'),'wall-plan-dialog');planDialog.setAttribute('aria-label','Update our plan');
 let planEdit=null;
 const planHeading=el('h3','',planDialog),planForm=el('form',null,planDialog),planLabel=el('label',null,planForm,'formfield'),planLabelText=el('span','',planLabel),planNote=el('textarea',null,planLabel),planTime=el('input',null,planLabel),planHint=el('small','',planForm,'muted'),planError=el('p','',planForm,'wall-error');
 planNote.dir='auto';planNote.maxLength=1000;planNote.rows=3;planTime.type='datetime-local';planError.setAttribute('role','alert');
 const planButtons=el('div',null,planForm,'row'),planSave=el('button','Send',planButtons);planSave.type='submit';planSave.className='primary';const planCancel=btn('Cancel',planButtons,()=>planDialog.close());planCancel.type='button';
 planDialog.onclick=e=>{if(e.target===planDialog&&!planSave.disabled)planDialog.close();};
 function editPlan(i,mode){
  planEdit={id:i.id,revision:i.revision,mode};planNote.value='';planTime.value='';planError.textContent='';
  planHeading.textContent=mode==='alternative'?'Suggest another idea':'Suggest a time';planLabelText.textContent=mode==='alternative'?'What would you like to do instead?':'Day and time';
  planNote.hidden=mode!=='alternative';planNote.required=mode==='alternative';planTime.hidden=mode==='alternative';planTime.required=mode!=='alternative';
  planHint.textContent=mode==='alternative'?'Your partner can reply in the comments. Edit the post once you agree on a different idea.':'Your time zone: '+localZone()+'. Your partner sees their local time.';
  planDialog.showModal();(mode==='alternative'?planNote:planTime).focus();
 }
 planForm.onsubmit=async e=>{e.preventDefault();if(planSave.disabled||!planEdit)return;planSave.disabled=true;planCancel.disabled=true;try{
  const {id,revision,mode}=planEdit;await act('item.plan',{id,revision,...(mode==='alternative'?{action:'respond',value:'alternative',note:planNote.value}:{action:'propose',at:new Date(planTime.value).toISOString()})});planDialog.close();
 }catch(err){planError.textContent=err.message;await sync().catch(()=>{});const current=latest(planEdit.id);if(current)planEdit.revision=current.revision;}finally{planSave.disabled=false;planCancel.disabled=false;}};
 function renderPlan(i,card,who){
  const p=planState(i),both=people.every(n=>p.responses[n]?.value==='yes'),time=p.schedule,confirmed=both&&time&&people.every(n=>time.confirmed.includes(n));
  const root=el('section',null,card,'wall-plan-agreement');root.setAttribute('aria-label','Our plan');
  const waiting=people.filter(n=>!p.responses[n]);
  let status=confirmed?'Agreed · time confirmed':both?'Choosing a time':waiting.length?'Waiting for '+waiting.join(' & '):Object.values(p.responses).some(r=>r.value==='alternative')?'Alternative suggested':'Not now';
  if(i.done)status='Completed';el('strong',status,root,'wall-plan-state');
  for(const n of people){const r=p.responses[n];el('p',n+' · '+({yes:'I’m in',alternative:'Suggested an alternative',later:'Not now'}[r?.value]||'Awaiting reply'),root,'wall-plan-person');if(r?.note)el('p',r.note,root,'wall-plan-note').dir='auto';}
  if(i.link&&/^https?:\/\//i.test(i.link)){const a=el('a','Open plan link ↗',root,'wall-plan-link');a.href=i.link;a.target='_blank';a.rel='noopener noreferrer';}
  if(!i.done){const replies=el('div',null,root,'wall-plan-actions');for(const [value,label]of [['yes','I’m in'],['alternative','Suggest another idea'],['later','Not now']]){
   const b=action(label,replies,()=>value==='alternative'?editPlan(latest(i.id),'alternative'):act('item.plan',{id:i.id,revision:i.revision,action:'respond',value}));b.setAttribute('aria-pressed',String(p.responses[who]?.value===value));if(value!=='alternative')b.disabled=p.responses[who]?.value===value;
  }}
  if(time){const section=el('div',null,root,'wall-plan-time');el('p',planDate(time.at),section);el('small','Your time · '+localZone(),section,'muted');el('small',confirmed?'Confirmed by both':time.by+' proposed this time · waiting for confirmation',section,'muted');}
  if(!i.done){const controls=el('div',null,root,'wall-plan-actions');if(time&&both&&!time.confirmed.includes(who)&&Date.parse(time.at)>Date.now())action('Confirm time',controls,()=>act('item.plan',{id:i.id,revision:i.revision,action:'confirm',scheduleId:time.id}),'primary');btn(time?'Suggest another time':'Suggest a time',controls,()=>editPlan(latest(i.id),'time'));}
 }
 ...
 function paint(){
  const state=getState();if(!state)return;draftBanner();paintDaily(state.daily);
 ...
 function render(i,card,who){
  ...
  if(i.type==='Plan')renderPlan(i,card,who);
 ...
 function fields(){const type=$('#item-type').value;$('#wall-steps-editor').hidden=type!=='Plan';$('#wall-plan-editor').hidden=type!=='Plan';$('#wall-plan-time-field').hidden=!!editing.id;$('#wall-agreement-note').hidden=type!=='Agreement';$('#wall-text-label').textContent=type==='Photo'?'Caption · optional':type==='Agreement'?'What are we agreeing to?':'Your moment';}
 function previews(){const root=$('#wall-previews');root.replaceChildren();photos.forEach((id,k)=>{const row=el('div',null,root);const image=el('img',null,row);image.src='/api/photos/'+id;image.alt='Selected photo '+(k+1);const remove=btn('×',row,()=>{photos.splice(k,1);previews();saveDraft();});remove.disabled=uploading;remove.setAttribute('aria-label','Remove photo '+(k+1));});}
 function edit(item={type:'Idea',title:''}){loadDrafts();saveDraft();const saved=postDrafts[item.id||'new'];if(saved)item=saved.editing;searchDialog.close();settingsDialog.close();epoch++;postRequest=null;editing=structuredClone(item);photos=[...images(item)];$('#item-form').reset();const select=$('#item-type');select.replaceChildren();categories.forEach(c=>{const option=el('option',(icons[c]||'')+' '+c,select);option.value=c;});select.value=item.type;$('#item-text').value=item.title||'';$('#item-ai').checked=!!item.aiAllowed;$('#wall-plan-link').value=item.link||'';$('#wall-steps').value=(item.steps??[]).map(s=>s.text).join('\n');$('#wall-editor-title').textContent=item.id?'Edit our moment':'Add a moment';$('#wall-publish').textContent=item.id?'Save changes':'Post to our wall';$('#wall-upload-status').textContent='';if(saved){photos=[...saved.photos];select.value=saved.type;$('#item-text').value=saved.title;$('#wall-steps').value=saved.steps;$('#wall-plan-link').value=saved.link||'';$('#wall-plan-time').value=saved.time||'';$('#item-ai').checked=!!saved.ai;postRequest=saved.receipt||null;}else{$('#wall-plan-time').value='';}draftActive=true;$('#wall-draft-status').textContent=saved?'Your draft is restored.':'';fields();previews();goto('editor');}
 $('#item-type').onchange=()=>{fields();saveDraft();};$('#add-item').onclick=()=>edit();$('#cancel-item').onclick=()=>history.back();$('#wall-add-photo').onclick=()=>$('#wall-upload').click();
 $('#wall-upload').onchange=async()=>{const input=$('#wall-upload'),files=[...input.files],ticket=epoch;if(!files.length)return;uploading=true;$('#wall-publish').disabled=true;$('#wall-add-photo').disabled=true;previews();try{if(photos.length+files.length>6)throw Error('Choose up to 6 photos.');for(const file of files){if(!['image/jpeg','image/png','image/webp'].includes(file.type)||file.size>5*1024*1024)throw Error('Choose JPG, PNG or WebP photos up to 5 MB each.');$('#wall-upload-status').textContent='Preparing photo…';const bitmap=await createImageBitmap(file),scale=Math.min(1,1600/Math.max(bitmap.width,bitmap.height)),canvas=document.createElement('canvas');canvas.width=Math.round(bitmap.width*scale);canvas.height=Math.round(bitmap.height*scale);canvas.getContext('2d').drawImage(bitmap,0,0,canvas.width,canvas.height);bitmap.close();const data=canvas.toDataURL('image/jpeg',.82).split(',')[1];const result=await api('photos',{data});if(ticket!==epoch)return;photos.push(result.id);previews();saveDraft();}$('#wall-upload-status').textContent='Photos ready.';}catch(err){$('#wall-upload-status').textContent=err.message;error(err);}finally{uploading=false;input.value='';$('#wall-publish').disabled=false;$('#wall-add-photo').disabled=false;previews();}};
 $('#item-form').onsubmit=async e=>{e.preventDefault();if(uploading)return;const b=$('#wall-publish');if(b.disabled)return;b.disabled=true;try{const type=$('#item-type').value,title=$('#item-text').value;if(!title.trim()&&!photos.length)throw Error('Write something or add a photo.');if(type==='Photo'&&!photos.length)throw Error('Add a photo for this moment.');const remaining=[...(editing.steps??[])];const steps=$('#wall-steps').value.split('\n').map(t=>t.trim()).filter(Boolean).map(text=>{const index=remaining.findIndex(s=>s.text===text);return index>=0?remaining.splice(index,1)[0]:{text,done:false};});const payload={...(type==='Plan'?{link:$('#wall-plan-link').value.trim(),...(!editing.id&&$('#wall-plan-time').value?{proposedAt:new Date($('#wall-plan-time').value).toISOString()}:{})}:{}),id:editing.id,revision:editing.revision,source:editing.source,type,title,images:photos,image:photos[0]??null,steps,aiAllowed:$('#item-ai').checked},key=JSON.stringify(payload);if(postRequest?.key!==key)postRequest={key,id:crypto.randomUUID()};saveDraft();await command('item.save',payload,postRequest.id);clearDraft();await sync();setFilter('All');$('#space-search').value='';goto('space',true);history.replaceState({placeTab:'space'},'');info('Saved to your wall.');}catch(err){error(err);}finally{b.disabled=false;}};
 ...
 return {paint,edit,saveDraft,openNotification,delta(d){const post=latest(d.post),c=post?.comments?.find(c=>c.id===d.id);if(c)c.text+=d.text;if(threadId===d.post){const body=[...threadList.querySelectorAll('[data-comment]')].find(n=>n.dataset.comment===d.id);if(body)renderComment(body,c??{by:'Echo',text:d.text,status:'streaming'});}},reset(){draftOwner=null;postDrafts={};draftActive=false;planDialog.close();discardDialog.close();sheet.close();reactionDialog.close();threadId=null;searchDialog.close();settingsDialog.close();dailySignature='';deleteId=null;deletion.close();epoch++;cards.clear();drafts.clear();editing={};photos=[];viewer.close();full.removeAttribute('src');}};
};
