...
window.OurWall=function({getState,api,command,sync,goto,info,error,el,btn,categories,getFilter,setFilter,openSource}){
 ...
 sheet.addEventListener?.('close',()=>{drafts.set(threadId,commentInput.value);threadId=null;threadSignature='';threadTrigger?.focus?.({preventScroll:true});});
 ...
 function openComments(id,trigger){threadId=id;threadTrigger=trigger;threadSignature='';replyReceipt=null;commentInput.value=drafts.get(id)||'';threadError.textContent='';paintThread();fitSheet();sheet.showModal();sheetClose.focus();threadList.scrollTop=threadList.scrollHeight;}
