...
export function course(seed,count=6){const r=random(seed),parts=[];let x=240;const order=[0,4,2,3,5,1];for(let i=order.length-1;i>1;i--){const j=1+Math.floor(r()*i);[order[i],order[j]]=[order[j],order[i]];}for(let i=0;i<count;i++){const type=order[i%6],length=650+Math.floor(r()*65),a=x+150;parts.push({i,type,start:x,end:x+length,a,b:a+([125,230,260,32,280,245][type]),period:1.6+r()*.45,phase:r()*6.28});x+=length;}return{version:2,seed,parts,finish:x+160};}
...
export function platforms(c,t,p){const out=[];let from=0;for(const q of c.parts){if(q.type===3)continue;out.push({x:from,w:q.a-from,y:FLOOR,id:'land'+q.i});if(q.type===1){out.push({x:(q.a+q.b)/2-55+Math.sin(t*1.6+q.phase)*32,w:110,y:FLOOR-12,id:'move'+q.i});}if(q.type===2){for(let k=0;k<6;k++){const id='crumble'+q.i+'-'+k;if(p.collapsed[id]===undefined||t-p.collapsed[id]<.24)out.push({x:q.a+k*(q.b-q.a)/6,w:(q.b-q.a)/6,y:FLOOR,id,crumble:true});}}if(q.type===4){out.push({x:q.a-35,w:q.b-q.a+70,y:FLOOR-62,id:'shortcut'+q.i});for(let k=0;k<4;k++)out.push({x:q.a+k*75,w:42,y:FLOOR+42,id:'steps'+q.i+'-'+k});}if(q.type===5){out.push({x:q.a-60,w:60,y:FLOOR,id:'boost'+q.i,boost:true});}from=q.b;}out.push({x:from,w:c.finish+300-from,y:FLOOR,id:'end'});return out;}
...
export function botInput(c,p,t,input){let dir=1,jump=input.jump;const q=c.parts.find(q=>q.b+35>p.x);
 if(q){if(q.type===3){if(p.x>q.a-70&&p.x<q.a&&!gateOpen(q,t+.18))dir=0;}else if(p.ground){const surfaces=platforms(c,t,p),s=surfaces.find(s=>s.id===p.on);if(q.type===4&&p.x>q.a-95&&p.x<q.a-45)jump++;else if(s&&s.x+s.w-p.x<24){const reach=p.boost>0?330:195;const landing=surfaces.find(n=>n.x>s.x&&n.x-p.x<reach);if(landing)jump++;else dir=0;}if(q.type===2&&p.x>q.a-30&&p.x<q.b)jump++;}}
 return{dir,jump};
}
