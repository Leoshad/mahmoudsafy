...
function sound(kind){if(!setting('sound')||audio?.state!=='running')return;try{const start=audio.currentTime;[0,.17].forEach((offset,i)=>{const o=audio.createOscillator(),g=audio.createGain();o.type=kind==='electric'?'sawtooth':'sine';o.frequency.value=kind==='need'?880-i*220:kind==='angry'?160:kind==='kiss'?620:kind==='sad'?440-i*110:kind==='miss'?440+i*110:520+i*180;g.gain.setValueAtTime(0,start+offset);g.gain.linearRampToValueAtTime(.055,start+offset+.02);g.gain.exponentialRampToValueAtTime(.001,start+offset+.3);o.connect(g);g.connect(audio.destination);o.start(start+offset);o.stop(start+offset+.32);});}catch{}}
...
async function send(kind){if(!who)return;remember(kind);unlock();const current=epoch;close();try{const result=await api('buzz',{id:crypto.randomUUID(),kind});if(epoch!==current)return;receive(result.event);}catch(e){if(epoch===current)info(e.message||'Buzz could not be sent.');}}
...
const settings=document.createElement('div');settings.className='buzz-settings';picker.append(settings);
for(const [key,title]of [['sound','Sound'],['vibration','Vibration']]){const label=document.createElement('label'),input=document.createElement('input');input.type='checkbox';input.checked=setting(key);input.onchange=()=>{try{localStorage.setItem('our-place:buzz:'+key,input.checked?'on':'off');}catch{}if(input.checked)unlock();};label.append(input,document.createTextNode(title));settings.append(label);}
...
function showNext(){if(layer||!queue.length||!who||!canDisplay())return;const event=queue.shift();active=event;const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;layer=document.createElement('div');layer.className='buzz-overlay buzz-'+event.kind;layer.setAttribute('role','status');layer.setAttribute('aria-label',kinds[event.kind][1]+' from '+event.from);
 ...
 else if(event.kind==='sad')art.innerHTML='<svg viewBox="0 0 200 200" aria-hidden="true"><defs><radialGradient id="buzz-sad-face" cx=".4" cy=".3"><stop stop-color="#ffe0ab"/><stop offset="1" stop-color="#d89d7b"/></radialGradient><linearGradient id="buzz-tear" x2="0" y2="1"><stop stop-color="#d9f5ff"/><stop offset="1" stop-color="#78b9ef"/></linearGradient></defs><circle cx="100" cy="96" r="67" fill="url(#buzz-sad-face)"/><path d="M55 72q13 5 26-8m38 0q13 13 26 8" fill="none" stroke="#825b56" stroke-width="5" stroke-linecap="round"/><path d="M58 92q12 12 25 0m34 0q12 12 25 0" fill="none" stroke="#674849" stroke-width="5" stroke-linecap="round"/><path d="M83 132q17-12 34 0" fill="none" stroke="#825b56" stroke-width="5" stroke-linecap="round"/><path class="buzz-tear" d="M139 97c-3 7-10 14-10 20a10 10 0 0 0 20 0c0-6-7-13-10-20Z" fill="url(#buzz-tear)"/><path class="buzz-tear-glint" d="M136 112q-3 5-1 8" fill="none" stroke="#effcff" stroke-width="2" stroke-linecap="round"/></svg>';
 else if(event.kind==='miss')art.innerHTML='<svg viewBox="0 0 200 200" aria-hidden="true"><defs><radialGradient id="buzz-miss-face" cx=".4" cy=".3"><stop stop-color="#ffe4b6"/><stop offset="1" stop-color="#e7ac80"/></radialGradient><linearGradient id="buzz-miss-heart" x2="0" y2="1"><stop stop-color="#ffb7d1"/><stop offset="1" stop-color="#d7548a"/></linearGradient></defs><path class="buzz-longing-heart" d="M100 182 22 102C-14 55 49 8 100 54c51-46 114 1 78 48Z" fill="#ef8fb344"/><circle cx="100" cy="88" r="60" fill="url(#buzz-miss-face)"/><path d="M58 58q13 1 24-8m36 0q11 9 24 8" fill="none" stroke="#865849" stroke-width="4" stroke-linecap="round"/><g fill="#533c40"><ellipse cx="76" cy="82" rx="14" ry="18"/><ellipse cx="124" cy="82" rx="14" ry="18"/></g><g fill="#fff4e6"><circle cx="72" cy="75" r="6"/><circle cx="120" cy="75" r="6"/><circle cx="81" cy="88" r="3"/><circle cx="129" cy="88" r="3"/></g><g fill="#ea8e91" opacity=".55"><ellipse cx="57" cy="103" rx="12" ry="6"/><ellipse cx="143" cy="103" rx="12" ry="6"/></g><path d="M93 113q7-6 14 0" fill="none" stroke="#875343" stroke-width="3" stroke-linecap="round"/><g class="buzz-held-heart"><path d="m100 177-37-37c-22-27 13-46 37-20 24-26 59-7 37 20Z" fill="url(#buzz-miss-heart)"/><path class="buzz-hug-left" d="M48 124q3 32 39 29" fill="none" stroke="#f6c795" stroke-width="14" stroke-linecap="round"/><path class="buzz-hug-right" d="M152 124q-3 32-39 29" fill="none" stroke="#f6c795" stroke-width="14" stroke-linecap="round"/></g></svg>';
