...
window.OurComfort?.init({sync:async()=>{const hold=tab==='chat'?PlaceScroll.hold($('#timeline')):null;try{await sync();if(state&&source?.readyState!==1)connect();}finally{hold?.restore();}},command,api,info});
