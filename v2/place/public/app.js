...
function connect(){if(!state||navigator.onLine===false)return;if(source&&source.readyState!==2&&Date.now()-lastEvent<45000)return;clearTimeout(reconnectTimer);reconnectTimer=null;source?.close();source=new EventSource('/api/events?draw=segments');lastEvent=Date.now();const stream=source,epoch=sessionEpoch;const listen=(type,fn)=>stream.addEventListener(type,e=>{if(source!==stream||epoch!==sessionEpoch)return;lastEvent=Date.now();fn(e);});listen('shared-touch',e=>window.OurTouch?.receive(JSON.parse(e.data).moment));listen('buzz',e=>window.OurBuzz?.receive(JSON.parse(e.data)));listen('typing',e=>receiveTyping(JSON.parse(e.data)));listen('files-changed',()=>window.OurFiles?.changed());
 listen('heartbeat',()=>lastEvent=Date.now());listen('open',()=>{window.OurNotifications?.refreshPresence?.();window.OurTouch?.connection(true);clearTimeout(reconnectTimer);reconnectTimer=null;recoveryFailures=0;recoveryAfter=0;paintPresence(false);drainOutbox();});listen('draw-segment',e=>window.OurDraw?.segment(JSON.parse(e.data)));listen('session-ended',signOutUI);listen('snapshot',e=>{lastEvent=Date.now();absorb(JSON.parse(e.data));});listen('delivered',e=>{if(!state)return;const r=JSON.parse(e.data);for(const m of [...older,...state.messages])if(r.ids.includes(m.id))m.deliveredAt=r.at;paintFeed();});listen('read',e=>{if(!state)return;const r=JSON.parse(e.data);applyReadReceipt(r);paintFeed();});listen('presence',e=>{presenceReceivedAt=Date.now();online=JSON.parse(e.data).online;paintPresence();});listen('delta',e=>{const d=JSON.parse(e.data),m=state?.messages.find(m=>m.id===d.id);if(m){const f=$('#timeline'),mark=tab==='chat'?PlaceScroll.capture(f):null;m.text+=d.text;const p=document.getElementById('text-'+d.id);if(p&&mark){renderMessageText(p,m);window.OurChatTools?.highlight();}if(mark)PlaceScroll.restore(f,mark);}});listen('wall-delta',e=>wall.delta(JSON.parse(e.data)));listen('notice',e=>{const text=JSON.parse(e.data).text;if(text!=='Echo stopped.')error(text);});source.onerror=()=>{if(source!==stream||epoch!==sessionEpoch)return;window.OurTouch?.connection(false);typingUntil=0;paintTyping();if(reconnectTimer)return;reconnectTimer=setTimeout(()=>{reconnectTimer=null;if(source?.readyState!==1&&state){paintPresence(false);window.OurMedia?.connection(false);}},1500);};}
...
document.addEventListener('visibilitychange',()=>{if(document.hidden)recoverConnection.resumePending=true;else recoverConnection(true);});window.addEventListener('focus',()=>recoverConnection(true));window.addEventListener('online',()=>recoverConnection(true));window.addEventListener('pageshow',e=>{if(e.persisted){recoverConnection.resumePending=true;recoverConnection(true);}});
