...
function paintQuote(root,message){
 ...
 const text=el('span',null,root,'reply-copy');el('strong',message.author,text);el('span',message.text?.slice(0,140)||(message.audio?'Voice message':message.image?'Photo':'Message'),text);
...
function paintAttachment(){const p=$('#attachment');p.replaceChildren();p.hidden=!attachment;$('#send').disabled=!!attachment&&!attachment.id;if(attachment){if(attachment.kind==='audio')el('span','♫ '+attachment.name,p);else{const im=el('img',null,p);im.src=attachment.preview||'/api/photos/'+attachment.id;im.alt='Photo to send';}if(!attachment.id)el('span',attachment.failed?'Upload failed · remove or choose the file again':attachment.kind==='audio'?'Uploading audio…':'Uploading photo…',p);btn('Remove',p,()=>{if(attachment.preview)URL.revokeObjectURL(attachment.preview);attachment=null;paintAttachment();});}}
...
$('#composer').onsubmit=async e=>{e.preventDefault();if(sending||!state)return;if(attachment&&!attachment.id){error('Wait for the attachment to finish uploading, or remove it before sending.');return;}const value=$('#compose').value.trim()||(attachment?.kind==='audio'?attachment.name:'');if(!value&&!attachment)return;sendTyping(false);sending=true;const audio=attachment?.kind==='audio'?attachment.id:undefined,img=audio?undefined:attachment?.id,ask=$('#ask').checked,m={id:crypto.randomUUID(),aiId:crypto.randomUUID(),author:state.who,createdAt:new Date().toISOString(),text:value,image:img,audio,audioMime:audio?'audio/mpeg':undefined,reply:reply?.id,status:'sending',payload:{text:value,image:img,audio,reply:reply?.id},ask,inspect:!!attachment?.ai};if(!saveOutbox(m)){sending=false;error('Could not save message. Your text is still here.');return;}$('#compose').value='';$('#compose').focus?.({preventScroll:true});if(attachment?.preview)URL.revokeObjectURL(attachment.preview);attachment=null;reply=null;paintReply();paintAttachment();sending=false;pending.set(m.id,m);paintFeed();await drainOutbox();};
