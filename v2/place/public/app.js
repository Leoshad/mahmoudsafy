...
async function openSource(id){const epoch=sessionEpoch;const m=[...older,...(state?.messages??[]),...pending.values()].find(x=>x.id===id)||await api('messages/'+encodeURIComponent(id));if(epoch!==sessionEpoch||!state)return;if(!m)throw Error('The original message is unavailable.');older=[...new Map([...older,m].map(x=>[x.id,x])).values()].sort((a,b)=>a.sequence-b.sequence);goto('chat');paintFeed();const row=[...$('#feed').children].find(n=>n.dataset.message===id);row?.scrollIntoView({block:'center'});showSourceHighlight(row);}
