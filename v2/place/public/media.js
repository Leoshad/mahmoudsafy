...
async function choose(t){if(current()){await action('media.enqueue',{track:t});say('Added to your shared queue.');}else{if(shared?.participants.includes(who)){render();$('#media-session-access').scrollIntoView?.({block:'nearest'});say('Use Open session to return, or End session for both to start a new one.');return;}localStart=0;$('#media-results').replaceChildren();await mount(t,true);$('#media-player-area').scrollIntoView?.({block:'nearest'});render();}}
