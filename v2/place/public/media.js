...
function restore(){
 ...
 selected=track;localQueue=track&&!member&&Array.isArray(saved.queue)?saved.queue.filter(validSavedTrack).slice(0,20):[];
...
function renderRecent(){
 const sig=JSON.stringify(recentPlays)+!!selected+JSON.stringify(queued())+busy;if(sig===recentSignature)return;recentSignature=sig;
 ...
 for(const track of recentPlays){const b=button((selected?'+ Up next · ':'▶ ')+track.title+' · '+track.channel,root,()=>choose(track),'media-recent-item');markQueueButton(b,track,true);b.dir='auto';}
...
async function mount(track,autoplay=true){
 ...
 if(e.data===0&&(current()?.queue.length||!current()&&localQueue.length)){advanceQueue();return;}
...
async function action(type,data={}){const m=current()||shared;if(!m)throw Error('This session has ended.');if(busy)return;busy=true;render();try{await host.command(type,{session:m.id,revision:m.revision,...data});await host.sync();}catch(e){await host.sync().catch(()=>{});throw e;}finally{busy=false;applyPlayback();render();}}
async function nextTrack(){if(current())return action('media.next');if(busy||!localQueue.length)return;const t=localQueue.shift();localStart=0;await mount(t,true);render();}
async function advanceQueue(){if(!(current()?.queue.length||!current()&&localQueue.length)||!ready||!allowed||!visible()||busy||current()&&loadedId!==current().track.videoId||player.getPlayerState()!==0)return;try{await nextTrack();}catch{await host.sync().catch(()=>{});}}
...
function render(){
 ...
 $('#media-leave').hidden=!m;$('#media-end').hidden=!m;$('#media-next').hidden=!(m?m.queue.length:localQueue.length);$('#media-sync').hidden=!m;$('#media-close').hidden=!selected;$('#media-close').disabled=busy;$('#media-title').title=selected?.title||'';scheduleLayout();
 const queue=m?m.queue:localQueue;const sig=(m?m.id:selected?'local':'')+JSON.stringify(queue);if(queueSignature!==sig){queueSignature=sig;const root=$('#media-queue');root.replaceChildren();if(selected){node('h3','Up next · '+queue.length,root);if(!queue.length)node('p','Find another song and tap + Up next. It will play after this one.',root,'muted');for(const [i,t] of queue.entries()){const row=node('div',undefined,root,'media-queue-row');node('span',t.title+(t.by?' · '+t.by:''),row);button('Remove',row,()=>{if(current())return action('media.remove',{queueId:t.queueId});localQueue.splice(i,1);render();});}}}
 $('#media-queue').hidden=!selected;for(const {b,t} of resultButtons)markQueueButton(b,t);renderRecent();persist();

}
async function invite(){if(busy)return;try{await host.sync();if(current()){await action('media.invite');return;}if(shared?.participants.includes(who)){render();say('Open or end your shared session above first.');return;}if(shared?.invitation?.to===who&&shared.invitation.expires>Date.now()+clockOffset){render();say('Accept or decline the invitation above before sending your own.');$('#media-session-access').scrollIntoView?.({block:'nearest'});return;}if(!selected)return;busy=true;const t=selected;await host.command('media.create',{mode,track:t,queue:localQueue,position:ready?player.getCurrentTime():0,...(shared?{replaceSession:shared.id,replaceRevision:shared.revision}:{})});await host.sync();if(shared?.owner===who){activeId=shared.id;localQueue=[];allowed=true;}say('Invitation sent to '+other()+'.');}catch(e){await host.sync().catch(()=>{});fail(e);}finally{busy=false;applyPlayback(true);render();}}
async function join(){if(!shared)return;const m=shared;await measureClock();if(m.invitation?.to===who)await action('media.join',{invitation:m.invitation.id});if(!shared||shared.id!==m.id||!shared.participants.includes(who))return;activeId=m.id;localQueue=[];mode=shared.mode;host.goto('together');pane().hidden=false;$('#media-entry').hidden=false;setMode(mode);await mount(shared.track,true);render();}
...
async function choose(t){if(busy)return;if(current()){const existing=queued().find(q=>q.videoId===t.videoId);await action(existing?'media.remove':'media.enqueue',existing?{queueId:existing.queueId}:{track:t});say(existing?'Removed from Up next.':'Added to Up next · plays after the current song.');if(!existing)await advanceQueue();}else{if(shared?.participants.includes(who)){render();$('#media-session-access').scrollIntoView?.({block:'nearest'});say('Use Open session to return, or End session for both to start a new one.');return;}if(selected){const index=localQueue.findIndex(q=>q.videoId===t.videoId);if(index>=0){localQueue.splice(index,1);render();say('Removed from Up next.');return;}if(localQueue.length>=20)throw Error('The queue has 20 videos. Remove one first.');localQueue.push(t);render();say('Added to Up next · plays after the current song.');await advanceQueue();return;}localStart=0;$('#media-results').replaceChildren();resultButtons=[];await mount(t,true);$('#media-player-area').scrollIntoView?.({block:'nearest'});render();}}
async function find(e){e.preventDefault();const q=$('#media-query').value.trim();if(!q)return;const ticket=++searchEpoch;$('#media-find').disabled=true;const isLink=/^(https?:\/\/|(?:www\.|m\.|music\.)?youtube\.com\/|youtu\.be\/)/i.test(q);say(isLink?'Checking video…':'Searching YouTube…');const root=$('#media-results');root.replaceChildren();resultButtons=[];scheduleLayout();try{let items;if(isLink){const r=await host.api('media/resolve?url='+encodeURIComponent(q));items=[r.track];}else{const r=await host.api('media/search?q='+encodeURIComponent(q));items=r.items;}if(ticket!==searchEpoch)return;persist();if(isLink&&!selected){await choose(items[0]);say('Press play, then invite '+other()+'.');return;}say(items.length?'Choose a result'+(selected?' to play after the current song.':'.'):'No playable results. Try another name.');for(const t of items){const row=node('article',undefined,root,'media-result');const img=node('img',undefined,row);img.src='https://i.ytimg.com/vi/'+t.videoId+'/mqdefault.jpg';img.alt='';img.loading='lazy';const text=node('div',undefined,row);node('strong',t.title,text).dir='auto';node('small',t.channel+' · '+time(t.duration),text).dir='auto';const b=button('',row,()=>choose(t));markQueueButton(b,t);resultButtons.push({b,t});}}catch(e){if(ticket===searchEpoch)fail(e);}finally{if(ticket===searchEpoch)$('#media-find').disabled=false;scheduleLayout();}}
function init(options){host=options;
 ...
 if(typeof ResizeObserver!=='undefined'){const layoutObserver=new ResizeObserver(scheduleLayout);layoutObserver.observe($('#media-player-anchor'));layoutObserver.observe($('#our-place-trial'));layoutObserver.observe($('#media-results'));layoutObserver.observe($('#media-panel'));layoutObserver.observe($('#media-session-access'));}
...
$('#media-search').onsubmit=find;document.querySelectorAll('[data-media-mode]').forEach(b=>b.onclick=()=>{host.goto('together');pane().hidden=false;setMode(b.dataset.mediaMode);$('#media-recent').open=false;render();host.revealPanel?.('#media-panel');});
 ...
 $('#media-next').onclick=()=>nextTrack().catch(fail);
 ...
 $('#media-close').onclick=async()=>{if(busy)return;try{if(current()){stopLocal();await action('media.leave');}activeId=null;selected=null;localQueue=[];localStart=0;destroy();pane().hidden=appTab==='chat';$('#media-player-area').hidden=true;$('#media-more').open=false;render();}catch(e){fail(e);}};
 $('#media-collapse').onclick=()=>{stopLocal();pane().hidden=true;$('#media-recent').open=false;positionPlayer();render();};

 $('#media-chat').onclick=()=>host.goto(appTab==='chat'?'together':'chat');
 ...
 setInterval(()=>{if(Date.now()-lastSavedAt>=5000)persist();advanceQueue();if(!current()||!ready||!allowed||!visible()||busy)return;const now=performance.now(),actual=player.getCurrentTime()||0,playing=player.getPlayerState()===1;
